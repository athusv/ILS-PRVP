...
Sol &Busca_local::best_improvement(Instance &grafo, Sol &S, mt19937 gen)
{
    // int count = 0;
    ...
    while (true){
        ...
        auto swap_inter_rotas = [&]() -> bool {
            return Busca_local::swap_inter_rotas(grafo, S, best_improvement);
        };
        auto swap_intra_rotas = [&]() -> bool {
            return Busca_local::swap_intra_rotas(grafo, S, best_improvement);
        };
        auto best_para = [&]() -> bool {
            return Busca_local::para(grafo, S, best_improvement);
        };
        auto swap_out_rotas = [&]() -> bool {
            return Busca_local::swap_out_rotas(grafo, S, best_improvement);
        };


        array<function<bool()>, 5> vizinhanca = {best_insert, swap_inter_rotas, swap_intra_rotas, best_para, swap_out_rotas};
        
        shuffle(vizinhanca.begin(), vizinhanca.end(), gen);
        
        
        if (!(vizinhanca[0]() || vizinhanca[1]() || vizinhanca[2]() || vizinhanca[3]()))
        {
            break;
        }
        // count++;
...
bool Busca_local::para(Instance &grafo, Sol &S, bool &best)
{
    ...
    for (int r = 0; r < S.rotas.size(); r++)
    {
        ...
        for (int i = 1; i < rota.route.size() - 1; i++)
        {
            ...
            if (grafo.score_vertices[rota.route[i]] < vertice_para[1] || grafo.score_vertices[rota.route[i]] < best_para[1])
                continue;
            ...
            if (possibilidade_parada)
            {
                ...
                if (best_para[1] < vertice_para[1])
                {
                    best_rota = r;
                    best_para = vertice_para;
                }
        ...
        for (int n = 0; n < 2; n++)
        {
            ...
            for (int i = 1; i < grafo.qt_vertices; i++)
            {
                ...
                for (int j = 0; j < rota.route.size() - 1; j++)
                {
                    ...
                    if (best_insert[1] == score_v && best_insert[2] < impacto)
                    {
                        continue;
                    }
                    ...
                    {
                        best_rota = r;
                        best_insert[0] = i;                                               // id vertice
                        best_insert[1] = score_v;                                         // score vertice
                        best_insert[2] = impacto;                                         // impacto incert vertice
                        best_insert[3] = j + 1;                                           // Local insert rota
                        best_insert[4] = rota.visita_custo[j] + dist1 + rota.plus_parada; // Visita_custo
                        best_insert[5] = (rota.plus_parada == grafo.t_parada) ? 1 : 0;
...
bool Busca_local::swap_inter_rotas(Instance &grafo, Sol &S, bool &best)
{
    double score_v1;
    double score_v2;
...
bool Busca_local::swap_intra_rotas(Instance &grafo, Sol &S, bool &best)
{
    double score_r1;
    double score_r2;
    ...
    if (best_rota1 != -1)
    {
        Caminho &rota = S.rotas[best_rota1];
        Caminho &rota2 = S.rotas[best_rota2];

        rota.excluir(swap_intra[0], S.visited_vertices, S.score, S.custo);
        rota2.excluir(swap_intra[1], S.visited_vertices, S.score, S.custo);

        rota.incert(swap_intra[2], S.visited_vertices, S.score, S.custo);
...
bool Busca_local::swap_out_rotas(Instance &grafo, Sol &S, bool &best)
{
